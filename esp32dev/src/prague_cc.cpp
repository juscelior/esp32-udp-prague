...
{
    uint64_t now64 = esp_timer_get_time();
    time_tp now = (time_tp)(now64 & 0xFFFFFFFFu);

    if (m_start_ref == 0) {
        m_start_ref = (now == 0 ? 1 : now);
        return 1;
    }
...
void PragueCC::GetTimeInfo(time_tp &timestamp,
                           ...
                           ecn_tp &ip_ecn)
{
    ...
    if (m_ts_remote != 0) {
        time_tp delta = timestamp - m_ts_remote;

        // avoid negative or zero values
        if (delta <= 0)
            delta = 1;

        echoed_timestamp = delta;
    } else {
        echoed_timestamp = 0;
    }
